...
#include "us_jp/heap.h"
#include "us_jp/load.h"
#include "us_jp/data.h"
#include "us_jp/seqplayer.h"
#include "us_jp/playback.h"
#include "us_jp/synthesis.h"
#elif defined(VERSION_EU)
#include "eu/port.h"
#include "eu/heap.h"
#include "eu/load.h"
#include "eu/data.h"
#include "eu/seqplayer.h"
#include "eu/playback.h"
#include "eu/synthesis.h"
#else
#include "sh/port.h"
#include "sh/heap.h"
#include "sh/load.h"
#include "sh/data.h"
#include "sh/seqplayer.h"
#include "sh/playback.h"
...
#ifdef VERSION_EU
OSMesgQueue OSMesgQueue0Data;
OSMesgQueue OSMesgQueue1Data;
OSMesgQueue OSMesgQueue2Data;
...
void audio_reset_session_eu(s32 presetId) {
    OSMesg mesg;
    osRecvMesg(OSMesgQueue3, &mesg, OS_MESG_NOBLOCK);
    osSendMesg(OSMesgQueue2, (OSMesg) presetId, OS_MESG_NOBLOCK);
    osRecvMesg(OSMesgQueue3, &mesg, OS_MESG_BLOCK);
...
void func_eu_802e9bec(s32 player, s32 channel, s32 arg2) {
    ...
    port_cmd_s8(0x08000000 | (player & 0xff) << 16 | (channel & 0xff) << 8, (s8) arg2);
}
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
                                    port_cmd_f32(0x02020000 | ((channelIndex & 0xff) << 8),
                                                  get_sound_volume(bank, soundIndex, VOLUME_RANGE_UNK1)
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
                                port_cmd_s8(0x03020000 | ((channelIndex & 0xff) << 8),
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
                                port_cmd_s8(0x05020000 | ((channelIndex & 0xff) << 8),
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
                            port_cmd_f32(0x02020000 | ((channelIndex & 0xff) << 8), 1);
                            port_cmd_s8(0x03020000 | ((channelIndex & 0xff) << 8), 64);
                            port_cmd_f32(0x04020000 | ((channelIndex & 0xff) << 8),
                                          get_sound_freq_scale(bank, soundIndex));
#else
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
    port_cmd_s8(0x46000000 | ((u8)(u32) player) << 16, seqId & SEQ_VARIATION);
    port_cmd_u32(0x82000000 | ((u8)(u32) player) << 16 | ((u8)(seqId & SEQ_BASE_ID)) << 8, arg2);
...
void seq_player_fade_out(u8 player, u16 fadeDuration) {
    ...
    u32 fd = fadeDuration;
#else
    s32 fd = fadeDuration; // will also match if we change function signature port_cmd_u32 to use s32 as arg1
#endif
    if (!player) {
        sCurrentBackgroundMusicSeqId = SEQUENCE_NONE;
    }
    port_cmd_u32(0x83000000 | (player & 0xff) << 16, fd);
#else
    if (player == SEQ_PLAYER_LEVEL) {
        sCurrentBackgroundMusicSeqId = SEQUENCE_NONE;
    }
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
            port_cmd_f32(0x01000000 | (player & 0xff) << 16 | (i & 0xff) << 8,
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
        if (muted) {
            port_cmd_u32(0xf1000000, 0);
        } else {
            port_cmd_u32(0xf2000000, 0);
        }
#else
...
void func_803210D4(u16 fadeDuration) {
    ...
    if (gSequencePlayers[SEQ_PLAYER_LEVEL].enabled == TRUE) {
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
        port_cmd_u32(0x83000000, fadeDuration);
#else
...
#if defined(VERSION_EU) || defined(VERSION_SH) || defined(VERSION_CN)
        port_cmd_u32(0x83010000, fadeDuration);
#else
